...
class ModelTrader:
    def __init__(self, ib_instance, model_path, state_file_path="trader_state.pkl", use_risk_management=True):
        ...
        Args:
            ib_instance: An IB client (or None for offline replay mode).
            model_path: Path to a bundle directory produced by train_current_model.py
                (containing best_model.zip, indicator_scaler.pkl, close_norm_params.pkl,
                lstm_generator.pkl, feature_order.json). For backward compatibility,
                a plain .zip path is also accepted and takes the legacy code path.
            state_file_path: Path to the pickle used for cross-session state.
            use_risk_management: Whether to attach bracket orders for SL/TP.
        """
        logger.info(f"Initializing ModelTrader with model at {model_path}")

        self.ib = ib_instance
        self.offline_mode = ib_instance is None
        if self.offline_mode:
            logger.info("ModelTrader running in OFFLINE mode (no IB connection)")
        ...
        self.pending_orders = {}
        self.locked_contract = None
        self.tick_data = {}
        self.current_bar = {}
        self.start_time = None
        self.use_ib_bars = True
        self.wait_for_bar_completion = True
        self.diagnostic_mode = True

        # Bundle-derived state (filled by load_model())
        ...
        # Legacy attrs retained for any residual legacy code paths
        ...
        if not hasattr(self, "is_recurrent_model"):
            self.is_recurrent_model = False
        self.lstm_states = None
        self.episode_starts = np.ones((1,), dtype=bool)

        # Live obs state (unrealized_pnl, time_in_position, drawdown)
        ...
        if self.feature_layout is not None:
            logger.info(
                f"ModelTrader ready: obs_dim={self.feature_layout['observation_dim']}, "
                f"{len(self.indicator_columns)} indicators, "
                f"recurrent={self.is_recurrent_model}, lstm_features={self.lstm_generator is not None}"
            )
        else:
            logger.warning(
                "ModelTrader initialized on legacy path (no feature_order.json found). "
                "Live obs will NOT include unrealized_pnl/time_in_position/drawdown."
            )
    ...
    def load_model(self):
        """Load the trained model + companion artifacts.

        Two layouts are supported:
        1. Bundle directory (preferred, produced by train_current_model.py):
           <dir>/best_model.zip, feature_order.json, indicator_scaler.pkl,
           close_norm_params.pkl, lstm_generator.pkl (optional).
        2. Legacy .zip path: falls back to loading just the model with no
           companion artifacts (obs will be incomplete — live is not supported).
        """
        logger.info(f"Loading model from {self.model_path}")
        try:
            bundle_dir = self._resolve_bundle_dir(self.model_path)
            if bundle_dir is not None:
                self.bundle_dir = bundle_dir
                # Load metadata first so inspection works even if the model is absent.
                self._load_feature_layout(bundle_dir)
                self._load_scaler_bundle(bundle_dir)
                self._load_lstm_generator(bundle_dir)

                model_zip = os.path.join(bundle_dir, "best_model.zip")
                if not os.path.exists(model_zip):
                    alt = os.path.join(bundle_dir, "model.zip")
                    model_zip = alt if os.path.exists(alt) else model_zip
                if not os.path.exists(model_zip):
                    logger.warning(
                        f"No model .zip found in bundle {bundle_dir}; "
                        f"metadata loaded but predictions will fail."
                    )
                    return True
            ...
            if self.model is not None:
                ...
                logger.info(f"Model observation space: {obs_dim}")
                if self.feature_layout and self.feature_layout["observation_dim"] != obs_dim:
                    logger.error(
                        f"Bundle obs_dim {self.feature_layout['observation_dim']} != "
                        f"model obs_dim {obs_dim}. Check that feature_order.json matches the model."
                    )
            return True
        except Exception as e:
    ...
    @staticmethod
    def _resolve_bundle_dir(model_path: str):
        """Return the bundle dir if model_path points to one, else None."""
        if os.path.isdir(model_path):
            if os.path.exists(os.path.join(model_path, "feature_order.json")):
                return model_path
            return None
        if model_path.endswith(".zip"):
            parent = os.path.dirname(model_path)
            if parent and os.path.exists(os.path.join(parent, "feature_order.json")):
                return parent
        return None

    def _load_feature_layout(self, bundle_dir: str):
        with open(os.path.join(bundle_dir, "feature_order.json"), "r") as f:
            self.feature_layout = json.load(f)
        self.indicator_columns = list(self.feature_layout["indicators"])
        # Legacy fields some call sites still expect:
        self.enabled_indicators = list(self.indicator_columns)
        self.required_features = ["close_norm"] + list(self.indicator_columns)
        logger.info(
            f"feature_layout: obs_dim={self.feature_layout['observation_dim']}, "
            f"indicators={len(self.indicator_columns)}"
        )

    def _load_scaler_bundle(self, bundle_dir: str):
        scaler_path = os.path.join(bundle_dir, "indicator_scaler.pkl")
        if not os.path.exists(scaler_path):
            logger.error(f"indicator_scaler.pkl not found in {bundle_dir}")
            return
        with open(scaler_path, "rb") as f:
            scaler_info = pickle.load(f)
        if isinstance(scaler_info, dict):
            self.scaler = scaler_info.get("scaler")
            self.scaler_cols = scaler_info.get("cols_to_scale", [])
        else:
            self.scaler = scaler_info
            self.scaler_cols = []
        logger.info(f"Loaded scaler ({type(self.scaler).__name__}) with {len(self.scaler_cols)} cols")

        close_path = os.path.join(bundle_dir, "close_norm_params.pkl")
        if os.path.exists(close_path):
            with open(close_path, "rb") as f:
                params = pickle.load(f)
            self.close_min = float(params["close_min"])
            self.close_max = float(params["close_max"])
            logger.info(f"Loaded close_norm params: min={self.close_min:.2f} max={self.close_max:.2f}")
        else:
            logger.warning(
                f"close_norm_params.pkl not found in {bundle_dir}; "
                f"close_norm will be computed per-call (may drift from training)."
            )
    ...
    def _load_lstm_generator(self, bundle_dir: str):
        ...
        if not os.path.exists(lstm_path):
            ...
            return
        if not LSTM_FEATURES_AVAILABLE:
            logger.error("LSTMFeatureGenerator import failed; cannot load lstm_generator.pkl")
            return
        try:
            gen = LSTMFeatureGenerator(lookback=20, hidden_size=32, num_layers=1, output_size=8)
            gen.load(lstm_path)
            self.lstm_generator = gen
    ...
    def preprocess_bar(self, bar: dict):
        """Build the observation vector for the current bar, matching the training env.

        Output layout (matches environment.py _get_obs):
            [close_norm, *indicator_columns, position, unrealized_pnl_norm,
             time_in_position_norm, drawdown_pct]

        Returns a 1-D numpy array of length feature_layout['observation_dim'],
        or None on failure.
        """
        if self.feature_layout is None:
            return self._preprocess_bar_legacy(bar)

        try:
            self.bar_history.append(bar)
            max_history = max(500, self.min_bars_needed, 200 + (
                self.lstm_generator.lookback if self.lstm_generator else 0
            ))
            if len(self.bar_history) > max_history:
                self.bar_history = self.bar_history[-max_history:]

            hist_df = pd.DataFrame(self.bar_history)
            time_key = "time" if "time" in hist_df.columns else "timestamp"
            hist_df = hist_df.set_index(time_key)
            for col in ["open", "high", "low", "close", "volume"]:
                if col in hist_df.columns:
                    hist_df[col] = pd.to_numeric(hist_df[col], errors="coerce")

            processed = process_technical_indicators(hist_df)
            if processed is None or processed.empty:
                logger.error("Indicator computation returned empty frame")
                return None

            if self.lstm_generator is not None:
                try:
                    processed = self.lstm_generator.transform(processed)
                except Exception as e:
                    logger.error(f"LSTM generator transform failed: {e}")
                    return None

            if self.scaler is not None and self.scaler_cols:
                cols_present = [c for c in self.scaler_cols if c in processed.columns]
                if cols_present:
                    try:
                        processed.loc[:, cols_present] = self.scaler.transform(
                            processed[cols_present]
                        )
                    except Exception as e:
                        logger.error(f"Scaler.transform failed: {e}")
                        return None

            if self.close_min is not None and self.close_max is not None:
                rng = max(self.close_max - self.close_min, 1e-9)
                close_norm = float((processed["close"].iloc[-1] - self.close_min) / rng)
                close_norm = min(1.0, max(0.0, close_norm))
            else:
                close_norm = 0.5
                logger.warning("close_norm params missing; using 0.5")
            ...
            for col in self.indicator_columns:
                if col in processed.columns:
                    ...
                    if pd.isna(val):
                        val = 0.0
                    indicator_values.append(float(val))
                else:
                    indicator_values.append(0.0)
                    logger.warning(f"Missing indicator column in live df: {col}")
            ...
            obs = np.array(
                [close_norm] + indicator_values + [pos_signal, unrealized_norm, tip_norm, dd],
                dtype=np.float32,
            )

            expected = self.feature_layout["observation_dim"]
            if obs.shape[0] != expected:
                logger.error(
                    f"Obs shape mismatch: built {obs.shape[0]} values, expected {expected}"
                )
                return None

            obs = np.clip(obs, -1.0, 1.0)
            obs[0] = max(0.0, obs[0])  # close_norm in [0,1]
            return obs
        except Exception as e:
            logger.error(f"preprocess_bar failed: {e}")
            import traceback
            logger.error(traceback.format_exc())
            return None

    def _update_trade_state(self, current_close: float) -> None:
        """Update live obs state to mirror environment.py net_worth/max_net_worth logic.

        Flow:
        - Fresh entry (flat -> long/short): record entry_price, zero time_in_position.
        - Position close (non-zero -> 0): materialize unrealized PnL into realized_cash.
        ...
        if pos != 0 and self.entry_price:
            ...
            self.unrealized_pnl = (current_close - self.entry_price) * sign * self.point_value * abs(pos)
        else:
            self.unrealized_pnl = 0.0
        ...
        if self.session_peak_equity > 0:
            self.drawdown_pct = min(
                0.0,
                (self.session_equity - self.session_peak_equity) / self.session_peak_equity,
            )
        else:
            self.drawdown_pct = 0.0
    ...
    def _preprocess_bar_legacy(self, bar: dict) -> pd.DataFrame:
        """Legacy preprocessing path used when no feature_order.json is present.

        Returns a DataFrame whose columns are [close_norm, *indicators, position].
        Retained for backward compatibility; new live trading should always use
        the bundle path.
        """
    ...
    def _load_normalization_scalers(self):
        """Deprecated: scaler loading is now handled by load_model() from the bundle dir.

        This stub is retained for backward compatibility with any code path that
        still calls it directly.
        """
        if self.scaler is None:
            logger.warning("_load_normalization_scalers called but no bundle scaler loaded")
    ...
    def get_prediction(self, obs):
        """Run the model on the observation vector.

        Accepts either an ndarray (preferred, from the bundle path) or a
        1-row DataFrame (legacy path).
        """
        try:
            if obs is None:
                logger.error("Invalid input for prediction: None")
                return None

            if isinstance(obs, np.ndarray):
                prediction_input = obs.reshape(1, -1) if obs.ndim == 1 else obs
                expected = self.model.observation_space.shape[0]
                if prediction_input.shape[1] != expected:
                    logger.error(
                        f"Obs dim mismatch: got {prediction_input.shape[1]}, expected {expected}"
                    )
                    return None
            ...
            normalized_df = obs
            if normalized_df is None or getattr(normalized_df, "empty", True):
                logger.error("Invalid input for prediction (empty DataFrame)")
                return None
